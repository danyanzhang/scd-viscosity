function [G2N, G2D, D2N] = areaBetweenCurves(filename)
%clear;clc; close all
%filename = '202008CHC022';
...
hold on
xplot = linspace(0, 21, 100);
yGBT = hillEval(xplot, coeffGBT);
yNative = hillEval(xplot, coeffNative);
G2N = trapz(xplot, yGBT-yNative);
plot(xplot, yGBT-yNative)

if length(sample.experiments) == 3
    yDMSO = hillEval(xplot, coeffDMSO);
    D2N = trapz(xplot, yDMSO-yNative);
    G2D = trapz(xplot, yGBT-yDMSO);
    plot(xplot, yDMSO-yNative)
    plot(xplot, yGBT-yDMSO)
    legend('G2N', 'D2N', 'G2D', 'Location', 'Southeast')
else
    legend('G2N', 'Location', 'Southeast')
    D2N = NaN;
    G2D = NaN;
end
plot(xplot, xplot.*0,'k')
axis([0, 22, -0.5, 0.5])
